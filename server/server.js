...
const { sendDownloadLink, sendAllDownloadLinks } = require("./mailer.js")
const { linkContactAndItem, getContactPurchasedItems } = require("./contact.js")
...
app.get("/items", async (req, res) => {
  ...
  const purchasedItemsIds = (await getContactPurchasedItems(email)).map((item) => item.id)
  res.json(
    items.map((item) => {
      return {
        id: item.id,
        ...
        purchased: purchasedItemsIds.includes(item.id)
      }
    })
  )
})
...
function createCheckoutSession(item) {
  return stripe.checkout.sessions.create({
    ...
    cancel_url: `${process.env.CLIENT_URL}/store.html`
  })
}
